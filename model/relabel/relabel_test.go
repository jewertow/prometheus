...
func TestRelabel(t *testing.T) {
	tests := []struct {
		...
		{ // dual-stack parsing - IPv6 expected
			...
			relabel: []*Config{
				{
					SourceLabels: model.LabelNames{"__meta_kubernetes_pod_annotation_prometheus_io_port", "__meta_kubernetes_pod_ip"},
					Regex:        MustNewRegexp("(\\d+);(([A-Fa-f0-9]{1,4}::?){1,7}[A-Fa-f0-9]{1,4})"),
					Action:       Replace,
					Replacement:  "[$2]:$1",
